...
ui.animation = function () {
    ...
    function request(elementsArray) {
        [].forEach.call(elementsArray, function (el) {
            if (ui.utils.isVisible(el, 50)) {
                _addAnimationClass(el);
            } else {
                _removeAnimationClass(el);
            }
        });
    }
    ...
    function _addAnimationClass(element) {
        [].forEach.call(element.parentElement.parentElement.childNodes[1].getElementsByTagName('*'), function (el) {
            console.log(el);
            if (el.dataset && el.dataset.animation && !el.classList.contains(el.dataset.animation)) {
    ...
    function _removeAnimationClass(element) {
        [].forEach.call(element.parentElement.nextSibling.getElementsByTagName('*'), function (el) {
            if (el.dataset && el.dataset.animation && el.classList.contains(el.dataset.animation)) {
...
ui.utils = function () {
    ...
    function isVisible(element, visibility) {
        ...
        if (elementTopPosition >= 0 - visibilityOffset && elementTopPosition < window.innerHeight + visibilityOffset) {
            return true;
        } else {
            return false;
        }
