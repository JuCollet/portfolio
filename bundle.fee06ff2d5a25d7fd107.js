...
ui.animation = function () {
            ...
            if (el.dataset && el.dataset.animation) {
                el.dataset.animation.split(' ').forEach(function (animationName) {
                    ...
                    if (animationName === 'slideUp' || animationName === 'slideDown') {
                        _moveUpDown(element, elementMustBeAdded, animationName === 'slideUp');
                    }
                });
            }
        });
    ...
    function _moveUpDown(element, isHappening, haveToMoveUp) {
        ...
        if (isHappening) {
            if (haveToMoveUp) {
                console.log("translate(0px," + (element.dataset.move - window.scrollY) + "px)");
                element.style.transform = "translate(0px," + (element.dataset.move - window.scrollY) + "px)";
            }
        } else {
